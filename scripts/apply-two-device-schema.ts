...
const MIGRATION_FILE = path.join(__dirname, '../supabase/migrations/20260611000001_two_device_schema.sql');
...
async function main() {
  ...
  console.log('Checking migration status for 016-parent-qr-pairing...\n');
  ...
  if (!tableErr) {
    ...
    console.log('   pairing_tokens table exists');
    process.exit(0);
  }

  console.log('❌ Migration not yet applied.\n');
  console.log('   Missing: pairing_tokens table');
  ...
  if (accessToken) {
    ...
    try {
      ...
      const tmpFile = path.join(__dirname, '../.migration-tmp-two-device.sql');
      ...
      console.log('Re-run: npm run migrate:two-device to confirm.');
    } catch (e: any) {
      console.error('\n❌ Auto-apply failed:', e.message);
      console.log('Apply manually via Supabase Dashboard → SQL Editor using the file:');
      console.log('  supabase/migrations/20260611000001_two_device_schema.sql');
      process.exit(1);
    }
  } else {
    ...
    console.log('─'.repeat(60));
    console.log('\nAfter applying, re-run: npm run migrate:two-device');
    process.exit(1);
  }
}
